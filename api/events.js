...
router.get("/get-events", async (req, res) => {
  try {
    const [events] = await db.query(
      ...
      TIME_FORMAT(time, '%H:%i:%s') AS time, venue FROM events`
    );
    res.json(events);
  } catch (error) {
...
// Prevent GET browser error on /register
...
router.post("/register", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!eventExists.length) return res.status(404).json({ error: "Event not found!" });
    const event = eventExists[0];

    // 1️⃣ Single Pass Restriction
    ...
    let maxTeammates = 0;
    let allowSolo = true;
    if (eventId === 1) { maxTeammates = 1; allowSolo = true; }
    else if ([6, 7, 8, 9].includes(Number(eventId))) { maxTeammates = 3; allowSolo = false; }
    else if ([2, 3, 4, 5].includes(Number(eventId))) { maxTeammates = 0; allowSolo = true; }

    // 4️⃣ Process teammates
    ...
    // 5️⃣ Teammates validation
    for (const t of validTeammates) {
      if (await hasSinglePassRestriction(t))
        return res.status(403).json({ error: `Teammate ${t} has single pass and is already registered.` });
      if (await isAlreadyRegistered(t, eventId))
        return res.status(400).json({ error: `Teammate ${t} already registered!` });
      const [existingUser] = await db.query("SELECT id FROM users WHERE id = ?", [t]);
      if (!existingUser.length)
        return res.status(400).json({ error: `Teammate ${t} does not exist!` });
    }

    // 6️⃣ Insert into DB
    await Promise.all(allTeamMembers.map(id => db.query(
